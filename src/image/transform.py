...
def resize_image(image, size, how):
    """Resize image
    ...
    size: int
    how: {"scale", "square"}
        - scale : conserve the scale of the image. size parameter define the minimal between width and height
        - square : create a square image of shape (size, size)
    ...
    if how == "scale":
        if img_h <= img_l:
            image_resized = resize(
                image, (size, round(size * img_l / img_h)), anti_aliasing=True
            )
        else:
            image_resized = resize(
                image, (round(size * img_h / img_l), size), anti_aliasing=True
            )
    elif how == "square":
        image_resized = resize(image, (size, size), anti_aliasing=True)

    else:
        raise ValueError("Invalid parameter 'how'")
...
def split_image_by_size(image, sqr_img_size=10):
    ...
    img_pieces_array = np.zeros(
        (nbr_h_split * nbr_l_split, sqr_img_size, sqr_img_size, image.shape[2])
    )

    for i in range(nbr_h_split):
        for j in range(nbr_l_split):
            img_pieces_array[i * nbr_l_split + j] = image[
                i * sqr_img_size : i * sqr_img_size + sqr_img_size,
...
def split_image_by_nbr_split(image, nbr_h_split, nbr_l_split):
    ...
    img_pieces_array = np.zeros(
        (nbr_h_split * nbr_l_split, img_piece_h_size, img_piece_l_size, image.shape[2])
    )
    ...
    for i in range(nbr_h_split):
        for j in range(nbr_l_split):
            ...
            u_r = (
                i * img_piece_h_size + img_piece_h_size,
                j * img_piece_l_size + img_piece_l_size,
            )
            img_pieces_array[i * nbr_l_split + j] = image[
                l_l[0] : u_r[0], l_l[1] : u_r[1]
            ]
